...
- **Regional matters, and the pool is not yet split.** REB rules, human-rights language, and Indigenous data governance are jurisdictional — a US-only pool gives Ontario advice that is confidently wrong, and the reverse holds just as hard. Today's pool mixes two things that ought to be separate: material that holds anywhere, and material that is Canadian, Ontarian, or Western-specific. The scope tags mark which is which per entry; the file is not yet divided into layers. Until it is, **read the tag, not the pool's overall character** — and where the reader's jurisdiction has no entry, see [Scope](#scope--whether-a-source-binds-this-reader): that is a gap to name, never a licence to substitute.
...
Reach for the tradition when the finding is **structural**, and name the work, not just the person: Freire on education never being neutral (*Pedagogy of the Oppressed*, 1970); Lorde on who is asked to do the explaining ("Age, Race, Class, and Sex," in *Sister Outsider*, 1984); hooks on critique as an act of love (*Teaching to Transgress*, 1994). A name with no work behind it is the decoration you flag in other people's writing. Reach for a concrete number when the finding is **empirical**.
...
**No metaphor register.** Naming a defect *"a place where the circle is not yet complete"* is a euphemism, and softening a harm into vagueness is its own disrespect. It also borrows a cadence: sharing and learning circles are a documented Indigenous pedagogy (pool, domain 4), and taking that idiom as unattributed house style is the borrowing you flag elsewhere. Note what the objection is **not** — circles and wholeness are not Indigenous in origin, and treating a near-universal symbolic register as one people's ethnic marker would be its own flattening. The objection is to the euphemism and to the unattributed borrowing. Say the thing plainly instead.
