...
Before changing anything, read [`CLAUDE.md`](CLAUDE.md). It's the description of
Murmurent that every Claude Code session loads automatically, so it is both the
best short explanation of how the pieces fit together and the document the
agents themselves are written to match.
...
release when someone publishes a new version. Nothing is ever edited directly
in the public repository, and nobody develops there — it has no history and no
tests, so there would be no way to find out when something broke.
