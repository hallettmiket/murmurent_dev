...
def test_harvest_writes_log_and_register(tmp_path: Path) -> None:
    ...
    report = reports / "report_1.md"
    report.write_text(REPORT)
    assert cg.evaluate(_write_call(report), docs, "2026-08-29") == {
        "status": "harvested",
        "rows": 2,
    }
    register = (docs / cg.REGISTER_NAME).read_text()
...
def test_repeated_gap_raises_hit_count_and_ranks_first(tmp_path: Path) -> None:
    ...
    cg.evaluate(_write_call(reports / "report_1.md"), docs, "2026-08-29")
    second = REPORT.replace("| OCAP® | 3 | entry on backlog, unfetched | `blocked` |", "")
    (reports / "report_2.md").write_text(second)
    cg.evaluate(_write_call(reports / "report_2.md"), docs, "2026-08-30")

    rows = [
        line
        for line in (docs / cg.REGISTER_NAME).read_text().splitlines()
...
def test_log_is_append_only(tmp_path: Path) -> None:
    ...
    data = [
        line
        for line in (docs / cg.LOG_NAME).read_text().splitlines()
...
def test_missing_section_is_reported_not_silent(tmp_path: Path) -> None:
    ...
    assert cg.evaluate(_write_call(report), docs)["status"] == "missing-section"
...
def test_declared_but_empty_register_is_not_a_warning(tmp_path: Path) -> None:
    ...
    assert cg.evaluate(_write_call(report), docs)["status"] == "harvested"
